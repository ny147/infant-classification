...
class AudioFeature:
    ...
    def _extract_mfcc(self, n_mfcc=25):
        self.mfcc = librosa.feature.mfcc(self.y, sr=self.sr, n_mfcc=n_mfcc)

        mfcc_mean = self.mfcc.mean(axis=1).T
        mfcc_std = self.mfcc.std(axis=1).T
    ...
    def plot_spectrogram(self):
        ...
        librosa.display.specshow(self.mfcc, sr=self.sr, x_axis='time')
    #
    #     if save_local:
    #         self._save_local()
    #
    ...
    #     out_name = out_name.replace(".wav", "").split("\\")[2]
    #     filename = f"./data/{self.label}/{out_name}.pkl"
